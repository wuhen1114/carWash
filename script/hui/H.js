    ...
    if (typeof require === 'function' && typeof exports === 'object' && typeof module === 'object') {
        ...
        factory(window['H'] = {
            ...
            selectTip: function (element, items, okCallback, cancelCallback) {
                ...
                html += '<div class="H-select-list H-flexbox-vertical H-position-relative animated fadeInUp">';
                ...
                if (items && items.length > 0) {
                    for (var i = 0; i < items.length; i++) {
                        html += '<div class="H-select-item H-font-size-14 H-theme-font-color-black H-center-all H-height-45">' + items[i] + '</div>';
                    }
                }
                else {
                    html += '<div class="H-select-item H-font-size-14 H-theme-font-color-black H-center-all H-height-45">请选择</div>';
                }
                html += '</div>';
            ...
            toastCore: function (iconHTML, tipText, duration, animateName) {
                ...
                duration = Math.abs(that.isNumber(duration) ? Number(duration) : 2000);
                animateName = animateName ? animateName : "animated fadeIn";
            ...
            toastSuccess: function (duration,tipTxt) {
                var that = this;
                that.toastCore('<span class="H-display-block H-line-height-normal"><i class="H-iconfont H-icon-right H-font-size-42"></i></span>', tipTxt, duration);
            },
            toastError: function (duration,tipTxt) {
                var that = this;
                that.toastCore('<span class="H-display-block H-line-height-normal"><i class="H-iconfont H-icon-error H-font-size-42"></i></span>', tipTxt, duration);
            },
            toastLoading: function (duration,tipTxt) {
                var that = this;
                duration = Math.abs(that.isNumber(duration) ? Number(duration) : 1000000000);
                that.toastCore('<span class="H-display-block H-line-height-normal H-animate-rotate"><i class="H-iconfont H-icon-none H-font-size-42"></i></span>', tipTxt, duration);
            },
            closeToast: function () {
                var that = this;
                if (that.Ds("#H-toast-tip") && that.Ds("#H-toast-tip").length > 0) {
                ...
                //删除页面加载过度动画
                if (that.Ds("#H-loadPage") && that.Ds("#H-loadPage").length > 0) {
                	that.addClass(that.D("#H-loadPage"),"animated fadeOut");
                	setTimeout(function () {
                		that.D("body").removeChild(that.D("#H-loadPage"));
                    }, 1000);
                }
            },
